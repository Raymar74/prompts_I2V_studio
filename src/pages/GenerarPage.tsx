...
type Step = 'idle' | 'generating-guion' | 'generating-imagen' | 'generating-prompts' | 'done' | 'error'
...
export function GenerarPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const caption = await generateCaption(config, character, guion, form.plataforma)
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4 space-y-6">
      <div className="card p-5 space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={(e) =>
                updateForm('humorIntensidad', Number(e.target.value))
              }
              className="flex-1 accent-brand-500"
              disabled={step !== 'idle' && step !== 'done'}
            />
            <span className="text-sm text-white/50 w-8 text-right">
              {form.humorIntensidad}
            </span>
          </div>
        </div>
        ...
        {(step === 'generating-guion' || step === 'generating-imagen' || step === 'generating-prompts') && (
          <div className="flex items-center gap-3 justify-center py-4">
            <div className="spinner" />
            <span className="text-sm text-white/60">
              {step === 'generating-guion'
                ? 'Generando guión...'
                : step === 'generating-imagen'
                  ? 'Generando prompt de imagen base...'
                  : 'Generando prompts I2V y caption...'}
            </span>
          </div>
        )}
      </div>
